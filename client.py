...
def train(model,
    ...
    with torch_distributed_zero_first(LOCAL_RANK):
        data_dict = check_dataset(data)  # check if None
    train_id = "train_" +str(cid)
    val_id = "val_" +str(cid)
    train_path, val_path = data_dict[train_id], data_dict[val_id]
    nc = 1 if single_cls else int(data_dict['nc'])  # number of classes
    ...
    ### TO BE CORRECTED
    train_path = "datasets/VisDrone/"+train_path
    val_path = "datasets/VisDrone/"+val_path
    ...
    if RANK in [-1, 0]:
        ...
        if not resume:
            ...
            if plots:
                plot_labels(labels, names, round_dir)
    ...
    LOGGER.info(f'Image sizes {imgsz} train, {imgsz} val\n'
                ...
                f"Logging results to {colorstr('bold', round_dir)}\n"
...
opt = parse_opt()
model,pretrained,ckpt,exclude,csd,device,callbacks = prepare(opt)
